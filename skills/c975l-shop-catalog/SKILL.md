...
The same copy makes a **template** (`Product::$template`): `createTemplate` copies a product into one, the
product itself left untouched, and `createProduct` draws a new product from a template (`?template=` on the
`duplicate` url). A template is **hidden for good** — `setHidden(false)` cannot reveal it — so every public read
already skips it; the back-office lists templates apart, behind the `templates` global action, and offers no
*New* there, a template never being made from an empty form.
...
Prices wear PaymentBundle's `price` and `price-label` classes, so they read like every other price on
the site; the product card only narrows its own through `--price-size`.
...
`shop-currency`, `shop-shipping-country`, `shop-shipping-free`, `shop-return-days`, `shop-name` and `url-terms-of-sales`
