...
`getCatalogueUrl()` returns `shop_index` with a `#products` fragment, generated through ConfigBundle's
`LocalizedUrlGenerator::path()` so a customer reading `/en/` is sent back into the English shop: the basket's
...
Its unsubscribe line is an `html` block, the anchor built in `ShopEmailTemplateProvider::html()` around a
`{{ unsubscribe_url }}` the renderer escapes — the catalogue sentence stays prose. `FormSeeder` backfills
data blocks only, so a site seeded before this keeps its `text` block: delete its `back_in_stock` rows and
run `c975l:ui:email-templates:ensure` again.
...
- **Do not write markup into a `shop` catalogue sentence** — an anchor is built in the provider and handed to an `html` block, so a translator writes prose.
