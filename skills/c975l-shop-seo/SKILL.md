...
**The same titled urls feed UiBundle's site search**, and `ShopAiSearchCardProvider` draws a product an
answer read as the shop's card (`templates/ai_search/card.html.twig`, with the basket button when there is
a single item), matched through the router and read by `findAvailableBySlugs()` — never a product the
