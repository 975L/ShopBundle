...
class ProductTest extends TestCase
{
    ...
    public function testTrashingAProductHidesIt(): void
    {
    ...
    // Restoring gives back the content, not the audience: the product comes back hidden, to be read once before it is shown again
    public function testRestoringAProductLeavesItHidden(): void
    {
        $product = new Product()->setHidden(false)->setIsDeleted(true);
    ...
    {
        $this->assertFalse(new ProductItem()->isHidden());
    }

    // What the sheet lists, the back-office still holding them all
    public function testOnlyTheItemsNotSetAsideAreReadOffTheProduct(): void
    {
        $product = new Product();
        $product->addItem(new ProductItem()->setTitle('A3'));
        $product->addItem(new ProductItem()->setTitle('A4')->setHidden(true));

        $this->assertCount(2, $product->getItems());
        $this->assertCount(1, $product->getVisibleItems());
        $this->assertSame('A3', $product->getVisibleItems()->first()->getTitle());
    }
}
