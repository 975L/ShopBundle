...
class SkillsTest extends TestCase
{
    ...
    private const string SLUG_PATTERN = '#^shop-[a-z-]+$#';

    private const string NAME_PATTERN = '#^[a-z][a-z0-9]*(_[a-z0-9]+)+(\([^)]*\))?$#';

    private const string COMMAND_PATTERN = '/c975l:shop:[a-z-:]+/';

    private const string COMPONENT_PATTERN = '/<twig:c975LShop:([A-Za-z:]+)/';
    ...
    private function isDeclaredSomewhere(string $name): bool
    {
        ...
        if (null === $haystack) {
            ...
            foreach (array_unique($directories) as $directory) {
                $haystack .= $this->readRecursively($directory);
            }
        }
    ...
    // Every php/yaml/json/twig file under a directory, symlinked ones included, each real directory read once
    private function readRecursively(string $directory, array &$visited = []): string
    {
        ...
        foreach (new \DirectoryIterator($directory) as $entry) {
            ...
            if ($entry->isDir()) {
                $content .= $this->readRecursively($entry->getPathname(), $visited);

                continue;
            }

            if (\in_array($entry->getExtension(), ['php', 'yaml', 'json', 'twig'], true)) {
    ...
    {
        foreach ($this->loadSkills() as $directory => $content) {
            $this->assertSame(1, preg_match('/^\*\*Key source paths[^\n]*\n[^\n]*/m', $content, $matches), sprintf('%s lists no key source path', $directory));
    ...
    public function testEveryClassMemberQuotedExists(): void
    {
        foreach ($this->loadSkills() as $directory => $content) {
            foreach ($this->backtickedTokens($content, '#^(?!c975L\\\\|App\\\\)[A-Z][A-Za-z]*(\\\\[A-Z][A-Za-z]*)*(::[A-Za-z_]+(\(\))?)?$#') as $token) {
                ...
                // A constant may carry a type ("const array PRODUCT_KINDS"), which a bare needle would miss
                $needle = str_ends_with($member, '()')
                    ? '/function\s+' . preg_quote(substr($member, 0, -2), '/') . '\s*\(/'
                    : '/const\s+(?:[A-Za-z\\\\|?]+\s+)?' . preg_quote($member, '/') . '\b/';
                $this->assertSame(1, preg_match($needle, $source), sprintf('%s quotes "%s", which %s does not hold', $directory, $token, basename($file)));
            }
        }
    }
