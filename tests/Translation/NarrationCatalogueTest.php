...
namespace c975L\ShopBundle\Tests\Translation;
...
class NarrationCatalogueTest extends TestCase
{
    ...
    public function testEveryDeclaredNarrationIsShippedInBothLocales(): void
    {
        ...
        foreach (self::LOCALES as $locale) {
            ...
            foreach ($declared as $file => $keys) {
                foreach ($keys as $key) {
                    $this->assertContains($key, $catalogue, sprintf('"%s" declares "%s", which "shop_narration.%s.xlf" does not ship.', $file, $key, $locale));
                }
            }
        }
    }
    ...
    public function testNoNarrationIsShippedWithoutBeingDeclared(): void
    {
        ...
        foreach ($this->catalogue('en') as $key) {
            $this->assertContains($key, $declared, sprintf('"shop_narration.en.xlf" ships "%s", which nothing in "src/" declares any more.', $key));
        }
    }
    ...
    {
        $path = $this->root() . '/translations/shop_narration.' . $locale . '.xlf';
